...
var Control = L.Control.extend({
    ...
    _drawSearch: function () {
        ...
        input.setAttribute(
            "placeholder",
            "Ingresa el lote hacia donde te diriges"
        );
        ...
        L.DomEvent.on(
            ...
            function (e) {
                ...
                cutFindPoints.map(function (_finded, index) {
                    ...
                    const liP =
                        document.getElementsByClassName("itemSearched")[index];
    ...
    _downloadGPX: function () {
        if (this.routeGeoJSON) {
            ...
            var trackPoints = this.routeGeoJSON.geometry.coordinates.map(
                function (coordinate) {
                    return {
                        "@lat": coordinate[1],
                        "@lon": coordinate[0],
    ...
    _createLocalizationList: function (container) {
        ...
        localizationSelect.setAttribute(
            "title",
            _this._local["Select language"]
        );
        L.DomEvent.on(
            localizationSelect,
            "change",
